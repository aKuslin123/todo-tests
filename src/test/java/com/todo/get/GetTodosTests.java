...
public class GetTodosTests extends BaseTest {
    ...
    public void testGetTodosWhenDatabaseIsEmpty() {
        ...
        Assertions.assertTrue(todos.isEmpty(), "Список TODO не пуст, хотя база данных пуста");
    }
    ...
    public void testGetTodosWithExistingEntries() {
        ...
        Assertions.assertEquals(2, todos.size(), "Список TODO должен содержать 2 записи");

        // Проверяем, что задачи присутствуют в списке
        boolean containsTodo1 = todos.stream().anyMatch(t -> t.getId() == todo1.getId());
        boolean containsTodo2 = todos.stream().anyMatch(t -> t.getId() == todo2.getId());

        Assertions.assertTrue(containsTodo1, "Задача 1 отсутствует в списке TODO");
        Assertions.assertTrue(containsTodo2, "Задача 2 отсутствует в списке TODO");
    }
